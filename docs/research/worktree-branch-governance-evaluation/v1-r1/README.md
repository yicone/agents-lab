...
Status: **frozen; no trials run**. This directory defines the first scored matrix for the experimental `worktree-branch-governance-audit` skill. Any behavior-affecting change to the frozen inputs ends this matrix and requires a new evaluation version and 15 new scored trials.
