...
5. **Validate before publishing.** Keep the response outside the repository or in a mode-0600 temporary file, parse JSON, and run `scripts/validate_review.py`. Reject malformed JSON, missing path/line/side, out-of-diff locations, duplicates, secrets, or executable shell text.
