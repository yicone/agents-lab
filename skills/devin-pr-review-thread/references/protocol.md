...
Consuming agents submit only the queue request through `scripts/queue_client.py`; they do not create request filenames, authorization records, or provider stdin payloads themselves. The queue client writes a mode-0600 request atomically and polls the matching response file.
