...
Consuming agents submit only the queue request through `scripts/queue_client.py`; they do not create request filenames, authorization records, or provider stdin payloads themselves. The queue client writes a mode-0600 request atomically and polls the matching response file. All normalized application statuses, including `await-user`, are emitted on stdout with exit code 0; callers must parse JSON before deciding whether the review succeeded. Host maintainers use `scripts/host_setup.py` for one-time exact-root plus worktree-parent enrollment, and `scripts/cleanup.py` for audited retention cleanup; neither command is part of the consuming-agent contract.
